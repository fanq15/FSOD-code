...
def single_scale_rpn_losses(
    ...
    neg_labels_int32 = rpn_labels_int32.new_full(rpn_labels_int32.shape, -1.0)
    neg_labels_int32[(rpn_labels_int32 == 0.0)] = 0.0 
    neg_labels_int32[(rpn_labels_int32 == 1.0)] = 0.0

    neg_bbox_targets = rpn_bbox_targets.new_full(rpn_bbox_targets.shape, 0.0)
    neg_rpn_bbox_inside_weights = rpn_bbox_inside_weights.new_full(rpn_bbox_inside_weights.shape, 0.0)
    neg_rpn_bbox_outside_weights = rpn_bbox_outside_weights.new_full(rpn_bbox_outside_weights.shape, 0.0)

    rpn_labels_int32 = torch.cat((rpn_labels_int32, neg_labels_int32), 0)
    rpn_bbox_targets = torch.cat((rpn_bbox_targets, neg_bbox_targets), 0)
    rpn_bbox_inside_weights = torch.cat((rpn_bbox_inside_weights, neg_rpn_bbox_inside_weights), 0)
    rpn_bbox_outside_weights = torch.cat((rpn_bbox_outside_weights, neg_rpn_bbox_outside_weights), 0)
