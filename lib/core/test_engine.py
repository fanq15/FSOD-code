...
def test_net_on_dataset(
    ...
    if multi_gpu:
        ...
        all_boxes, all_segms, all_keyps = multi_gpu_test_net_on_dataset(
            args, dataset_name, proposal_file, num_images, num_classes, output_dir, dataset
        )
    else:
        all_boxes, all_segms, all_keyps = test_net(
            args, dataset_name, proposal_file, output_dir, dataset, gpu_id=gpu_id
        )
    test_timer.toc()
...
def test_net(
    ...
    full_roidb, dataset, start_ind, end_ind, total_num_images, total_num_cls, support_dict = get_roidb_and_dataset(
        dataset_name, proposal_file, ind_range, dataset
    )
    ...
    for i, entry in enumerate(roidb):
        ...
        query_cls = list(set(entry['gt_classes']))[0] #index_pd.loc[index_pd['index']==index, 'cls_ls'].tolist()[0]
        query_img = entry['image'] #index_pd.loc[index_pd['index']==index, 'img_ls'].tolist()[0]
...
def get_roidb_and_dataset(dataset_name, proposal_file, ind_range, dataset):
    ...
    if cfg.TEST.PRECOMPUTED_PROPOSALS:
        ...
        original_roidb, roidb = dataset.get_roidb(
    ...
    if ind_range is not None:
        ...
        start, end = ind_range
        #base_real_index = roidb[start]['real_index']
        #last_real_index = roidb[end-1]['real_index']
        #roidb = roidb[start:end]
        #for item in roidb:
        #    item['real_index'] -= base_real_index
    else:
        start = 0
        end = len(roidb)
        total_num_images = end
    
    #print(base_real_index, last_real_index)
