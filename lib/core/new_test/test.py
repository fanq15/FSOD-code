...
def im_detect_all(model, im, support_data, support_box, support_cls, support_shot, query_path, box_proposals=None, timers=None):
    ...
    #print(all_cls) 
    '''
    for i in range(int(support_data.shape[0] / support_shot)):
        if cfg.TEST.BBOX_AUG.ENABLED:
            scores, boxes, im_scale, blob_conv = im_detect_bbox_aug(
                model, im, box_proposals)
        else:
            scores, boxes, im_scale, blob_conv = im_detect_bbox(
                model, im, support_data[i*support_shot:(i+1)*support_shot], support_box[i*support_shot:(i+1)*support_shot], support_cls, query_path, cfg.TEST.SCALE, cfg.TEST.MAX_SIZE, box_proposals)
        #print(scores.shape, boxes.shape, support_cls[i])
        if i == 0:
            all_scores = scores
            all_boxes = boxes
            all_cls = np.array([support_cls[i]] * scores.shape[0])
        else:
            all_scores = np.concatenate((all_scores, scores), axis=0)
            all_boxes = np.concatenate((all_boxes, boxes), axis=0)
            all_cls = np.concatenate((all_cls, np.array([support_cls[i]] * scores.shape[0])))
    '''
    all_cls = all_cls[:, np.newaxis]
    #print(all_cls)
    #print(all_scores.shape, all_boxes.shape, scores.shape, boxes.shape)
    ...
    scores, boxes, cls_boxes, nms_cls = box_results_with_nms_and_limit(all_scores, boxes, all_cls)
    #print(nms_cls)
...
def im_detect_bbox(model, im, support_data, support_box, support_cls, query_path, target_scale, target_max_size, boxes=None):
    ...
    if cfg.TEST.BBOX_REG:
        ...
        #print(box_deltas.shape, boxes.shape)
