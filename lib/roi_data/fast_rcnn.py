...
def _sample_rois(roidb, im_scale, batch_idx):
    ...
    total_fg_inds = np.where(max_overlaps >= cfg.TRAIN.FG_THRESH)[0]
    # Guard against the case when an image has fewer than fg_rois_per_image
    # foreground RoIs
    fg_rois_per_this_image = np.minimum(fg_rois_per_image, total_fg_inds.size)

    # target_cls fg
    target_fg_inds = np.where((max_overlaps >= cfg.TRAIN.FG_THRESH) & (roidb['max_classes'] == roidb['target_cls']))[0]

    # Sample foreground regions without replacement
    if target_fg_inds.size >= fg_rois_per_this_image:
        fg_inds = npr.choice(
            target_fg_inds, size=fg_rois_per_this_image, replace=False)
    else:
        other_fg_inds = np.array(list(set(total_fg_inds) - set(target_fg_inds)))
        other_fg_rois_per_this_image = fg_rois_per_this_image - target_fg_inds.size
        other_fg_inds = npr.choice(
            other_fg_inds, size=other_fg_rois_per_this_image, replace=False)
        fg_inds = np.concatenate((target_fg_inds, other_fg_inds), axis=0).astype(np.int32)
        #print('other', other_fg_inds.shape, target_fg_inds.shape)
    #print(fg_inds, target_fg_inds.shape, other_fg_inds.shape)
    #print(fg_inds.shape)
...
def _compute_targets(ex_rois, gt_rois, labels):
    ...
    if cfg.MODEL.CLS_AGNOSTIC_BBOX_REG:
        labels_new = labels.copy().clip(max=1) #, out=labels)
    return np.hstack((labels_new[:, np.newaxis], targets)).astype(
